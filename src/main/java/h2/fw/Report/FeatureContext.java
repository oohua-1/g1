...
public class FeatureContext {
	...
	private final Map<Integer, com.epam.reportportal.cucumber.ScenarioContext> scenarios = new HashMap<>();
	private final Set<String> tags;

	private Maybe<String> id = Maybe.empty();
	private com.epam.reportportal.cucumber.RuleContext currentRule;

	private void handleNode(@Nonnull Deque<com.epam.reportportal.cucumber.RuleContext> ruleQueue, @Nonnull URI uri, @Nonnull Node node) {
		if (node instanceof Node.Rule) {
			Node.Rule rule = (Node.Rule) node;
			com.epam.reportportal.cucumber.RuleContext ruleContext = new com.epam.reportportal.cucumber.RuleContext(uri, (Node.Rule) node);
		...
		if (node instanceof Node.Scenario) {
			...
			scenarios.put(line, new com.epam.reportportal.cucumber.ScenarioContext(uri, ruleQueue.peekLast(), scenario));
		}
		if (node instanceof Node.ScenarioOutline) {
			...
			scenarioOutline.elements()
					...
					.forEach(e -> scenarios.put(e.getLocation().getLine(),
							new com.epam.reportportal.cucumber.ScenarioContext(uri, ruleQueue.peekLast(), scenarioOutline, e)
					));
		}
	}

	private <T extends Node> void handleNodes(@Nonnull URI uri, @Nonnull Collection<T> nodes) {
		Deque<com.epam.reportportal.cucumber.RuleContext> ruleQueue = new LinkedList<>();
	...
	@Nonnull
	public Optional<com.epam.reportportal.cucumber.RuleContext> getCurrentRule() {
